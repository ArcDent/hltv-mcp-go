...
export default function Players() {
  ...
  const search = async () => {
    if (!q.trim()) return
    setLoading(true)
    try { const r = await api.search(q, 'player'); setList(r?.items ?? []) } catch { setList([]) }
    setLoading(false)
  }
  ...
  return (
    <div className="anim-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', gap: 12 }}>
        <input placeholder="搜索选手 — 如 ZywOo、载物、s1mple"
      ...
      {list?.map((p, i) => (
        <div key={i} className="anim-in" style={{ ...card, animationDelay: `${i * 35}ms` }}>
          ...
          {p.slug && <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{p.slug}</span>}
        </div>
      ))}
    </div>
  )
